...
class terminalArt():
    ...
    def main(self):
        ...
        for CHANGER in range(im.n_frames):
            filename = tempfile.gettempdir() + "/frame" + str(CHANGER) + ".png"
            self.fn = tempfile.gettempdir() + "/frame" + str(CHANGER) + ".html"
            im.seek(CHANGER)
            im.save(filename)
